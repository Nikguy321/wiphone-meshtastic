...
| **Menu** (top-left soft key) | the map menu: download maps, go to coordinates, follow me, measure (an anchor at the crosshair; scroll away and the strip reads the distance and bearing), pins, places, nodes — the three lists are **nearest-first** with distance and bearing on every row |
