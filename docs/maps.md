...
| Key | What it does |
|---|---|
| **Arrows** | scroll. A tap is a nudge; **hold one down and it keeps scrolling and speeds up** (a repeat every 100 ms after 400 ms, through the same 24 → 48 → 72 → 96 px curve a run of taps climbs); let go and the next press is a nudge again. With **Snap to markers** on (the menu; it is on by default) a tap that would land on or fly past a pin, a place from the mesh or a node with a position **stops on it**, and a tap while sitting on one **jumps to the next marker that way** (within a 45° cone, on screen) — the arrows walk them. The strip says what you landed on: `Pin 3 (3 of 4)`, `hunt camp (place from the mesh)`, `Nick H (node, 12m ago)`. A hold never snaps; it is a sweep |
