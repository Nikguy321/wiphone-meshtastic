/* GENERATED by tools/gen_entities.py from Python's html.entities.html5 — do not edit.
 *
 * The same table COVEY's HTMLParser decodes with. An entity this misses would make a
 * chapter's extracted text longer than COVEY's, and since a reading position is a
 * character offset into that text, the error would grow through the chapter.
 *
 * 2125 entities, longest name 31 chars, widest expansion 6 bytes.
 */
#pragma once

#define HTML_ENTITY_MAX_NAME 31

struct HtmlEntity { const char* name; const char* utf8; };

// Sorted by name (strcmp order) so lookup is a binary search.
...
static const int HTML_ENTITY_N = 2125;
