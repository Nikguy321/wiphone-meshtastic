...
// Reported to the app benchmark: 1 if the buffer landed in fast internal RAM,
// 0 if it had to fall back to (slower) PSRAM.
...
{
	// VRAM first: it's read heavily during scanline rendering, so it benefits most
	// from internal RAM; grab that before the larger work-RAM claims the space.
	hw.vbanks = gb_alloc_pref_internal(2 * 8192, &gb_hw_vram_internal);
	hw.rambanks = gb_alloc_pref_internal(8 * 4096, &gb_hw_wram_internal);
